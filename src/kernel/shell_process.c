...
static void cmd_help(const char* args) {
    ...
    terminal_writestring("- spawn <name>: Create a new test process\n");
}
...
static void cmd_kill(const char* args) {
    ...
    if (space) {
        *space = '\0';
        char* confirm_arg = space + 1;
        if (strcmp(confirm_arg, "confirm") == 0) {
            confirm = 1;
        }
    }
...
static void cmd_spawn(const char* args) {
    ...
    strncpy(process_name, args, sizeof(process_name) - 1);
...
static const shell_command_t commands[] = {
    ...
    {"spawn", cmd_spawn, "Create a new test process"},
